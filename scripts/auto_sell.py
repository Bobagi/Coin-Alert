...
def process_fills(conn):
    print("[AUTO-FILL] Checking for filled limit orders...")
    with conn.cursor() as cur:
        cur.execute(
            """
    ...
    for buy_id, sell_id in rows:
        ...
        if order['status'] == 'FILLED':
            ...
            print(f"[AUTO-FILL] Order {sell_id} FILLED; released R${exec_quote:.8f}. locked R${locked:.8f}, remaining R${remaining:.8f}")
...
def process_sells(conn):
    ...
    if notional < min_notional:
        print(f"[AUTO-SELL] Grouped notional {notional} below minNotional {min_notional}, auto-buying to meet threshold")
        ...
        if buy_res.get('status') == 'success':
            ...
            locked    = get_current_exposure(conn)
            remaining = PURCHASE_LIMIT_QUOTE - locked
            print(f"[AUTO-SELL] Post-auto-buy locked R${locked:.8f}, remaining R${remaining:.8f}")
        return
...
def process_buys(conn, last_buy_time):
    locked    = get_current_exposure(conn)
    remaining = PURCHASE_LIMIT_QUOTE - locked
    print(f"[AUTO-BUY] Current exposure R${locked:.8f}, remaining buy power R${remaining:.8f}")

    now = datetime.now(timezone.utc)
    if last_buy_time and (now - last_buy_time).total_seconds() < BUY_DELAY_HOURS*3600:
        return last_buy_time
    if remaining <= 0:
        print(f"[AUTO-BUY] Exposure cap reached; skipping buy")
        return last_buy_time
    ...
    print(f"[AUTO-BUY] Buy R${increment} of {TRADE_SYMBOL}: {result}")
    ...
    if result.get('status') == 'success':
        ...
        locked    = get_current_exposure(conn)
        remaining = PURCHASE_LIMIT_QUOTE - locked
        print(f"[AUTO-BUY] After buy locked R${locked:.8f}, remaining R${remaining:.8f}")
        return now
    return last_buy_time
...
def main():
    ...
    last_buy_time = None
    print(f"Service start: cap R${PURCHASE_LIMIT_QUOTE}, incr R${BUY_INCREMENT_QUOTE}, delay {BUY_DELAY_HOURS}h UTC")
    while True:
        try:
            process_fills(conn)
            process_sells(conn)
