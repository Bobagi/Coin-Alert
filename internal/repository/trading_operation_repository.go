...
func (repository *PostgresTradingOperationRepository) ListRecentOperations(contextWithTimeout context.Context, limit int) ([]domain.TradingOperation, error) {
        ...
        for rows.Next() {
            ...
            scanError := rows.Scan(&operation.Identifier, &operation.TradingPairSymbol, &operation.QuantityPurchased, &operation.PurchasePricePerUnit, &operation.TargetProfitPercent, &operation.Status, &operation.SellPricePerUnit, &operation.PurchaseTimestamp, &operation.SellTimestamp)
            if scanError != nil {
