...
func NewBinanceSymbolService() *BinanceSymbolService {
    return &BinanceSymbolService{
        SymbolEndpoint: "https://api.binance.com/api/v3/exchangeInfo",
        HTTPClient: &http.Client{
...
func (service *BinanceSymbolService) FetchAvailableSymbols(fetchContext context.Context) ([]string, error) {
    ...
    if response.StatusCode != http.StatusOK {
        return nil, errors.New("binance symbols endpoint responded with non-OK status")
    }
